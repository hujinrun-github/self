...
func (r *Repository) SaveAdmin(ctx context.Context, input ProfileInput, ifMatch string) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT updated_at FROM profile WHERE id = 1`).Scan(&updatedAt); err != nil {
		return err
	}
	if etagFor(updatedAt) != ifMatch {
		return ErrConflict
	}

	now := r.clock().UTC().Format(time.RFC3339Nano)
	_, err = tx.ExecContext(ctx, `UPDATE profile SET name = ?, headline = ?, summary = ?, bio = ?, avatar_media_id = ?, email = ?, seo_title = ?, seo_description = ?, og_image_media_id = ?, updated_at = ? WHERE id = 1`,
		input.Name,
		input.Headline,
		input.Summary,
		input.Bio,
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM social_links WHERE profile_id = 1`); err != nil {
		return err
	}
	for index, link := range input.SocialLinks {
		_, err := tx.ExecContext(ctx, `INSERT INTO social_links (profile_id, label, url, icon, sort_order, created_at, updated_at) VALUES (1, ?, ?, ?, ?, ?, ?)`,
			link.Label,
			link.URL,
			link.Icon,
			(index+1)*10,
...
func (r *Repository) getProfile(ctx context.Context) (ProfileAdminDTO, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT id, name, headline, summary, bio, avatar_media_id, email, seo_title, seo_description, og_image_media_id, updated_at FROM profile WHERE id = 1`).
		Scan(&profile.ID, &profile.Name, &profile.Headline, &profile.Summary, &profile.Bio, &avatarID, &profile.Email, &profile.SEOTitle, &profile.SEODescription, &ogID, &profile.UpdatedAt)
	if err != nil {
		return ProfileAdminDTO{}, err
	}
	if avatarID.Valid {
...
func (r *Repository) getSocialLinks(ctx context.Context) ([]SocialLinkDTO, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id, label, url, icon, sort_order FROM social_links WHERE profile_id = 1 ORDER BY sort_order, id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
